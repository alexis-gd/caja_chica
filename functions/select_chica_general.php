...
function getPettyCashDetails()
{
    ...
    $stmt = $conexion->prepare("SELECT * FROM caja_chica WHERE id_caja = ?");
...
function getVoucher()
{
    ...
    $stmt = $conexion->prepare("SELECT id_comprobante FROM caja_chica WHERE id_caja = ?");
    ...
    $stmt_modelo = $conexion->prepare("SELECT id, nombre FROM modelo_chica_comprobante");
    $stmt_modelo->execute();
    $rows_modelo = $stmt_modelo->fetchAll();

    if (count($rows_modelo) == 0) {
        echo json_encode(['type' => 'ERROR', 'message' => 'Sin Resultados en modelo_chica_comprobante.']);
        return;
    }
...
function getFileType()
{
    ...
    $stmt = $conexion->prepare("SELECT * FROM modelo_chica_archivo WHERE band_eliminar = 1 ORDER BY nombre ASC");
    $stmt->execute();
    $rows = $stmt->fetchAll();
...
function getDashCaja()
{
    ...
    $stmt = $conexion->query("SELECT COUNT(*) as total FROM caja_chica WHERE band_eliminar = 1");
    $fila = $stmt->fetch();
