...
function fetchFillSelect(option, id_item, selectedId = null, option_value = '', typeSelect = null, base = 'functions/caja_general') {
  let datos = new FormData();
  datos.append('opcion', option);
  datos.append('option_value', option_value);

  fetch(base + '/select.php', {
    method: 'POST',
    body: datos
  })
    .then(response => response.json())
          ...
          if (!selectId) {
            ...
            $(selectElement).selectize({
              ...
              onOptionAdd: function (value) {
                ...
                handleNewOptionAdd(value, this, base);
              }
            });
          }
...
function handleNewOptionAdd(newOption, selectInstance, base = 'functions/caja_general') {
